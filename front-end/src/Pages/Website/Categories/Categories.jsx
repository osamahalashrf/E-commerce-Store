...
export default function WibSiteCategories() {
  ...
  const showCategories = categories.map((item, key) => (
    <div key={key} className="border-0">
      <div className=" m-1 bg-white border flex items-center justify-start gap-3 rounded py-2 h-full">
    ...
    <>
      <div className="bg-blue-200 py-5">
        <div className="container mx-6 py-6">
          <div className="flex items-stretch justify-center flex-wrap gap-y-2">
            {loading ? <SkeletonShow length="5" height="70px" baseColor="white" /> : showCategories}
          </div>
        </div>
      </div>
    </>
